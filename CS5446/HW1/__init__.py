...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        '''  
        agent_str = "(at pt" +  str(self.state.agent.position.x) + "pt" +  str(self.state.agent.position.y) + " agent1)"
        # predicate_str = ""
        # predicate part
        up_next_str = ""
        for i in range(self.width-1):
            for j in range(1,self.num_lanes):
                tmp_str = "(up_next pt{}pt{} pt{}pt{})".format(i+1,j,i,j-1)
                up_next_str = up_next_str + tmp_str
        down_next_str = ""
        for i in range(self.width-1):
            for j in range(self.num_lanes-1):
                tmp_str = "(down_next pt{}pt{} pt{}pt{})".format(i+1,j,i,j+1)
                down_next_str = down_next_str + tmp_str
        forward_next_str = ""
        for i in range(1,self.width):
            for j in range(self.num_lanes):
                tmp_str = "(forward_next pt{}pt{} pt{}pt{})".format(i,j,i-1,j)
                forward_next_str = forward_next_str + tmp_str
        predicate_str = up_next_str + down_next_str + forward_next_str

        # block part
        block_str = ""
        for car in self.state.cars:
            tmp_str = "(blocked pt{}pt{})".format(car.position.x,car.position.y)
            block_str = block_str + tmp_str
        
        return agent_str + up_next_str + down_next_str + forward_next_str + block_str
...
def generateDomainPDDLFile(gen):
    ...
    '''
    at_string = "(at ?p1 ?ag)"
    # next_string = " (OR (OR (up_next ?p1 ?p2) (down_next ?p1 ?p2)) (forward_next ?p1 ?p2))"
    up_next_string = " (up_next ?p1 ?p2)"
    down_next_string = "(down_next ?p1 ?p2)"
    forward_next_string = "(forward_next ?p1 ?p2)"
    block_string = " (not (blocked ?p2))"
    gen.addAction(name="UP",
                parameters=(("p1","gridcell"),("p2","gridcell"),("ag","agent")),
                precondition_string= "(and " + block_string + " " + "(and " + at_string + " " + up_next_string + "))",
                effect_string= "(and (not (at ?p1 ?ag)) (at ?p2 ?ag))")
    gen.addAction(name="DOWN",
                parameters=(("p1","gridcell"),("p2","gridcell"),("ag","agent")),
                precondition_string= "(and " + block_string + " " + "(and " + at_string + " " + down_next_string + "))",
                effect_string= "(and (not (at ?p1 ?ag)) (at ?p2 ?ag))")
    gen.addAction(name="FORWARD",
                parameters=(("p1","gridcell"),("p2","gridcell"),("ag","agent")),
                precondition_string= "(and " + block_string + " " + "(and " + at_string + " " + forward_next_string + "))",
                effect_string= "(and (not (at ?p1 ?ag)) (at ?p2 ?ag))")
