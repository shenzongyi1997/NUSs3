...
class Adamax(Optimizer):
    ...
    def update(self, w, w_grads, iteration):
        ...
        for k in list(w.keys()):
            print("key:",k," value:", w[k])
            ############## To Do ################
            self.momentum[k] = self.beta_1 * self.momentum[k] + (1-self.beta_1) * w_grads[k]
            # self.accumulators[k] = self.beta_2 * self.accumulators[k] + (1 - self.beta_2) * w_grads[k]**2
            print(self.accumulators[k].shape, type(self.accumulators[k]))
            for i in range(self.accumulators[k].shape[0]):
                for j in range(self.accumulators[k].shape[1]):
                    self.accumulators[k][i][j] = max(abs(w_grads[k][i][j]), self.beta_2 * self.accumulators[k][i][j])
            
            if True:
                momentum_t = self.momentum[k] / (1 - self.beta_1**(iteration+1))
                new_w[k] = w[k] - self.lr * momentum_t / (self.accumulators[k])
            else:
                new_w[k] = w[k] - self.lr * self.momentum[k] / (self.accumulators[k])
            #####################################
        return new_w
